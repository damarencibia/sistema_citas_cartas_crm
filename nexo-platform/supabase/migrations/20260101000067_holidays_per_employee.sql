-- =============================================================
-- Migration 000067: Holidays/exceptions per employee
-- =============================================================
-- - holiday_exceptions.employee_id: exceptions now belong to a
--   specific worker. NULL = applies to the whole business
--   (keeps existing global rows working).
-- - Role-aware RLS replaces the tenant-only policy:
--     * owner / admin: manage every exception of the tenant
--       (including business-wide NULL rows).
--     * employee: read their own exceptions + business-wide ones,
--       and may only insert/update/delete their own.
-- - get_full_slot_grid now blocks a worker's date only when the
--   exception belongs to them or is business-wide.
-- =============================================================

ALTER TABLE holiday_exceptions
  ADD COLUMN employee_id UUID REFERENCES employees(id) ON DELETE CASCADE;

-- Allow multiple employees to have an exception on the same date.
ALTER TABLE holiday_exceptions
  DROP CONSTRAINT IF EXISTS holiday_exceptions_tenant_id_date_key;

CREATE INDEX idx_holiday_exceptions_tenant_employee
  ON holiday_exceptions(tenant_id, employee_id, date);

-- Prevent duplicates within the same scope.
CREATE UNIQUE INDEX uq_holiday_exceptions_global_date
  ON holiday_exceptions(tenant_id, date)
  WHERE employee_id IS NULL;

CREATE UNIQUE INDEX uq_holiday_exceptions_employee_date
  ON holiday_exceptions(tenant_id, employee_id, date)
...
CREATE POLICY holiday_exceptions_admin_manage ON holiday_exceptions
  FOR ALL TO authenticated
  USING (
    tenant_id = (auth.jwt() -> 'app_metadata' ->> 'tenant_id')::uuid
    AND (auth.jwt() -> 'app_metadata' ->> 'role')::text IN ('owner', 'admin', 'super_admin')
  )
  WITH CHECK (
    tenant_id = (auth.jwt() -> 'app_metadata' ->> 'tenant_id')::uuid
    AND (auth.jwt() -> 'app_metadata' ->> 'role')::text IN ('owner', 'admin', 'super_admin')
...
CREATE POLICY holiday_exceptions_employee_read ON holiday_exceptions
  FOR SELECT TO authenticated
  USING (
    tenant_id = (auth.jwt() -> 'app_metadata' ->> 'tenant_id')::uuid
    AND (auth.jwt() -> 'app_metadata' ->> 'role')::text = 'employee'
    AND (
      employee_id IS NULL
      OR employee_id IN (
        SELECT e.id
...
CREATE POLICY holiday_exceptions_employee_write ON holiday_exceptions
  FOR ALL TO authenticated
  USING (
    tenant_id = (auth.jwt() -> 'app_metadata' ->> 'tenant_id')::uuid
    AND (auth.jwt() -> 'app_metadata' ->> 'role')::text = 'employee'
...
-- === Slot generation: per-employee holiday check ===
