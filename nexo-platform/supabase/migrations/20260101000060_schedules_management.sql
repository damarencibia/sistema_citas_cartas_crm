-- =============================================================
-- Migration 000060: Schedules management
-- =============================================================
-- Atomic save of an employee's weekly schedule via RPC.
-- - Replaces all rows for (tenant, employee) in a single transaction
--   (previous client-side DELETE + INSERT was not atomic).
-- - Role-based authorization:
--     * owner / admin: may edit any employee of the tenant or the
--       business default schedule (p_employee_id IS NULL).
--     * employee: may edit only their own schedule (matched through
--       users.supabase_user_id = auth.uid()).
-- - The RLS policy on `schedules` only enforces tenant isolation, so
--   without this guard any authenticated staff member could rewrite
--   another employee's schedule via the REST API.
-- =============================================================
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO schedules (
      ...
      advance_booking_days, min_advance_minutes
    )
    VALUES (
      v_tenant_id, p_employee_id, v_day, v_start, v_end,
      true, v_mode, v_interval, v_advance, v_min_advance
    );
